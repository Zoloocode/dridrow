        ...
        if results.multi_face_landmarks:
            ...
            if EAR < thresholds["EAR_THRESH"]:
                ...
                if self.state_tracker["DROWSY_TIME"] >= thresholds["WAIT_TIME"]:
                    ...
                    sound = AudioSegment.from_file("/Users/bdmccg/Desktop/Driver-Drowsiness-detection-using-Mediapipe-Source code/audio/wake_up.wav")
                    play(sound)
                    plot_text(frame, "SEREEREI! ZOLOO", ALM_txt_pos, self.state_tracker["COLOR"])
            ...
            EAR_txt = f"EAR haritsaa: {round(EAR, 2)}"
            DROWSY_TIME_txt = f"UNTALT: {round(self.state_tracker['DROWSY_TIME'], 3)} Sec(s)"
...
if __name__ == "__main__":
    # Create a VideoFrameHandler instance
    frame_handler = VideoFrameHandler()

    # Define the thresholds
    thresholds = {
        "EAR_THRESH": 0.2,  # You can adjust these thresholds
        "WAIT_TIME": 2.0,
    }

    # Replace CORRECT_DEVICE_INDEX with the correct device index for your external webcam
    CORRECT_DEVICE_INDEX = 0  # Change this to your external webcam's device index

    # Open a video capture source with the correct device index for your external webcam
    cap = cv2.VideoCapture(CORRECT_DEVICE_INDEX)
    ...
    while True:
        ...
        # Process the frame using your VideoFrameHandler
